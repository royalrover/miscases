...
(function(j) {  // j == JTopo
    function b(l) { //b采用了观察者模式，类似于Publisher类。
        var k = this;
        this.name = l;
        this.messageMap = {};
        this.messageCount = 0;
        this.subscribe = function(n, o) {   //subscribe(type,fn)
            if (!typeof n == "string") {
                m(n, o)
            } else {
        ...
        this.publish = function(n, q, p) {  //publish(type,args)
    ...
    Array.prototype.del = function(l) {
        if (typeof l != "number") {
            for (var k = 0; k < this.length; k++) {
                if (this[k] === l) {
                    return this.slice(0, k).concat(this.slice(k + 1, this.length))
                 // return this.splice(k-1,1)  返回产出元素组成的数组，splice修改 的是原数组对象
                }
            }
            return this
        } else {
 ...
 // 返回当前鼠标事件的页面坐标
    function h(k) { 
        if (k.pageX || k.pageY) { //ie系列的event没有pageX属性，ff，chrome，Safari，opera有
            return {
                x: k.pageX,
                y: k.pageY
            }
    ...
    function c(l) { //l是event对象
        l = l || h(window.event);
        var k = document.body.scrollLeft + (l.x || l.layerX); //ie中的x和非ie的layerX，返回鼠标在定位的包含元素（border开始）的位置，
    ...
    function i(n, m, q, o, l) {
        ...
        var p = Math.atan2(s, t) + l; //atan2()计算弧度，s为角度的对边边长，t为临边边长
        return {
            x: n + Math.cos(p) * k,
            y: m + Math.sin(p) * k
        }
   ...
   //JTopo的工具类
    j.util = {
        rotatePoint: i, // //旋转某一弧度，l可理解为旋转因子
        rotatePoints: d,
        getDistance: e,
        getXY: c,
        mouseCoords: h,// 返回当前鼠标的页面坐标
        MessageBus: b,
        isIE: (!+[1,]),
        clone: f  //浅克隆
    };
...
(function(a) {       //Jtopo.Stage   a==JTopo
    function b(e) {
        var d = this;
        a.stage = d;
        ...
        this.saveAsLocalImage = function() {
            var g = this.canvas.toDataURL("image/png").replace("image/png", "image/octet-stream");
...
(function(b) {         //     Jtopo.Scene
    function c(d) {             //Jtopo.AbstractScene
        ...
        this.paintBackgroud = function(f) {
            this.graphics.save();
            if (this.background) {
                this.graphics.drawImage(f, 0 - this.eagleEye.translate.x, 0 - this.eagleEye.translate.y, this.width, this.height)
            } else {
                this.graphics.fillStyle = "rgba(" + this.style.fillStyle + "," + this.alpha + ")";
                this.graphics.fillRect(-this.eagleEye.translate.x, -this.eagleEye.translate.y, this.width, this.height)
            }
        ...
        this.paint = function() {
            ...
            if (this.eagleEye.visible && !b.util.isIE) {
                h = this.getEagleEyeImage(this.eagleEye.width, this.eagleEye.height);
                f.translate(this.eagleEye.translate.x, this.eagleEye.translate.y)
            }
            ...
            if (this.eagleEye.visible && h != null && !b.util.isIE) {
                f.putImageData(h, this.width - this.eagleEye.width, this.height - this.eagleEye.height)
            }
            f.restore()
        };
        ...
        this.getelementByXY = function(f, k) {
            ...
            for (var h = this.elements.length - 1; h >= 0; h--) {
                ...
                if (g.translate) {
                    if (f > g.x && f < g.x + g.width * g.scalaX && k > g.y && k < g.y + g.height * g.scalaY) {
                        j = g;
                        break
                    }
                } else {
                    if (f > g.x && f < g.x + g.width && k > g.y && k < g.y + g.height) {
                        j = g;
                        break
                    }
                }
        ...
        this.updateEyeView = function(f, h) {
            var g = this.getBound();
            this.eagleEye.translate.x = (this.eagleEye.width - (this.width - f)) / this.eagleEye.width;
            this.eagleEye.translate.y = (this.eagleEye.height - (this.height - h)) / this.eagleEye.height;
        ...
        this.mousedownHander = function(g) {
            ...
            this.mouseDownX = f;
            this.mouseDownY = i;
            if (this.eagleEye.visible && f > this.width - this.eagleEye.width && i > this.height - this.eagleEye.height) {
        ...
        this.mousemoveHander = function(i) {
            ...
            if (this.isOnMouseDown) {
                if (this.eagleEye.visible && f > this.width - this.eagleEye.width && j > this.height - this.eagleEye.height) {
...
(function(j) {
    function h(k) {
        ...
        this.paint = function(l) {
            ...
            if (this.image) {
                ...
                l.rect(-this.width / 2, -this.height / 2, this.width, this.height);
                l.fill();
                l.closePath()
            }
            this.drawText(l)
    ...
    function d() {
        ...
        this.paint = function(k) {
            ...
            k.bezierCurveTo(this.x + 75, this.y + 37, this.x + 70, this.y + 25, this.x + 50, this.y + 25);
            k.bezierCurveTo(this.x + 20, this.y + 25, this.x + 20, this.y + 62.5, this.x + 20, this.y + 62.5);
            k.bezierCurveTo(this.x + 20, this.y + 80, this.x + 40, this.y + 102, this.x + 75, this.y + 120);
            k.bezierCurveTo(this.x + 110, this.y + 102, this.x + 130, this.y + 80, this.x + 130, this.y + 62.5);
            k.bezierCurveTo(this.x + 130, this.y + 62.5, this.x + 130, this.y + 25, this.x + 100, this.y + 25);
            k.bezierCurveTo(this.x + 85, this.y + 25, this.x + 75, this.y + 37, this.x + 75, this.y + 40);
            k.fill();
            k.closePath();
            k.restore()
        }
    ...
    function f(k) {
        ...
        this.paint = function(q) {
                ...
                for (var n in this.classObj) {
                    ...
                    if (typeof t == "function") {
                        ...
                        if (n.length + s.length > 35) {
                            this.operations.push("+ " + n + s.substring(0, 35 - n.length));
                            this.operations.push("  " + s.substring(35 - n.length))
                        } else {
            ...
            q.strokeText(this.className, this.x + (this.width - r) / 2, this.y + this.rowHeight);
            for (var p = 0; p < this.operations.length; p++) {
                var o = this.operations[p];
                q.strokeText(o, this.x + 5, this.y + this.rowHeight + this.rowHeight * (p + 1))
            }
            q.moveTo(this.x, this.y + this.rowHeight * (this.operations.length + 1) + 3);
            q.lineTo(this.x + this.width, this.y + this.rowHeight * (this.operations.length + 1) + 3);
    ...
    function b() {
        ...
        if (arguments.length <= 3) {
            ...
            k = new g(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5])
        }
        k.stop = function() {
            k.isStop = true
        };
...
(function(e) {
    function d(j, i, h) {
        ...
        this.paint = function(k) {
            ...
            if (j === i) {
                ...
                k.moveTo(this.nodeA.x + this.nodeA.width / 2, this.nodeA.y + this.nodeA.height / 2);
                k.lineTo(this.nodeZ.x + this.nodeZ.width / 2, this.nodeZ.y + this.nodeZ.height / 2)
            }
            k.stroke();
            k.closePath();
            this.paintText(k)
        ...
        this.paintText = function(l) {
            if (this.name) {
                ...
                l.font = this.style.fontSize + " " + this.style.font;
                l.strokeStyle = "rgba(" + this.style.fontColor + ", " + this.alpha + ")";
                l.strokeText(k, this.nodeA.x + (this.nodeZ.x - this.nodeA.x) / 2, this.nodeA.y + (this.nodeZ.y - this.nodeA.y) / 2 + 12);
                l.stroke();
                l.closePath()
            }
        }
    ...
    function g(j, i, h) {
        ...
        this.paint = function(l) {
            ...
            l.moveTo(this.nodeA.x + this.nodeA.width / 2, this.nodeA.y + this.nodeA.height / 2);
    ...
    function c(k, j, i, h) {
        ...
        this.paint = function(w) {
            ...
            if (n == m || r == q) {
                w.save();
                w.beginPath();
                w.strokeStyle = "rgba(" + this.style.strokeStyle + "," + this.alpha + ")";
    ...
    function b(k, j, h) {
        ...
        this.paint = function(m) {
            ...
            if (p == o || r == q) {
                m.moveTo(this.nodeA.x + this.nodeA.width / 2, this.nodeA.y + this.nodeA.height / 2);
                m.lineTo(this.nodeZ.x + this.nodeZ.width / 2, this.nodeZ.y + this.nodeZ.height / 2)
            } else {
                var n = (this.flexional.direction == "horizontal" ? this.flexional.extend() : 0);
                var l = (this.flexional.direction == "vertical" ? this.flexional.extend() : 0);
                n *= (o - p) > 0 ? 1 : -1;
                l *= (q - r) > 0 ? 1 : -1;
                m.moveTo(p + this.nodeA.width / 2, r + this.nodeA.height / 2);
                m.lineTo(p + this.nodeA.width / 2 + n, r + this.nodeA.height / 2 + l);
                m.lineTo(o + this.nodeA.width / 2 - n, q + this.nodeA.height / 2 - l);
    ...
    function a() {
        ...
        this.paint = function(f) {
            ...
            for (var e = 0; e < j.length - 2; e += 3) {
                ...
                f.quadraticCurveTo(j[e + 1].x, j[e + 1].y, j[e + 2].x, j[e + 2].y);
                f.stroke();
                f.closePath();
                f.save();
                f.beginPath();
...
(function(b) {
    ...
    function d(l, m) {
        ...
        var n = l.getCenterXY();
        if (j == "star") {
            h = a(n.x, n.y, m.length, l.layout.radius, l.layout.beginDegree, l.layout.endDegree)
        } else {
            if (j == "tree") {
                h = c(n.x, n.y, m.length, k.width, k.height, k.direction)
            } else {
                if (j == "grid") {
                    h = e(n.x, n.y, k.rows, k.cols, k.horizontal || 0, k.vertical || 0)
                } else {
                    return
                }
            }
    ...
    function b() {
        ...
        c.paint = function(e) {
            ...
            for (var f = 0; f < this.datas.length; f++) {
                ...
                e.arc(this.x + d / 2, this.y + g / 2, this.radius / 2, j, j + k, false);
                e.fill();
                e.closePath();
                j += k
            }
    ...
    function b(n, o, f, p, q) {
        ...
        for (var j in o) {
            ...
            i[j] = {
                ...
                step: l / f * h,
                isDone: function(s) {
                    var r = (this.step > 0 && n[s] >= this.targetValue) || (this.step < 0 && n[s] <= this.targetValue);
                    return r
                }
            }
        }
        ...
        function l(o, u, s, t, p) {
            ...
            q.paint = function(r) {
                ...
                if (this.image != null) {
                    ...
                    r.fillStyle = "rgba(" + this.style.fillStyle + "," + this.alpha + ")";

...
(function(Topo,undefined){
    ...
    utils.isInArray = function(a,arr){
        var i,len;
	...
	utils.getXYInCanvas = function(e,canvas){
        ...
        return {
            x: (e.clientX - loc.left) * (canvas.width / loc.width),
            y: (e.clientY - loc.top) * (canvas.height / loc.height)
        }
	};
...
//	Topo.namespace('Topo.Stage');
	function Stage(canvas){
		...
		this.saveAsImage = function(){
			...
			img.src = imgUrl;
			openwindow = window.open("_blank",'save image from canvas');
		...
		canvas.addEventListener('click',function(e){
            ...
            setTimeout(function(){stage.repaint(stage.graphic)},this.frame);
		});
		...
		this.repaint = function(graphic){
			if(!("visiable" in this.currentScene) || !this.currentScene.visiable) return;
...
/**
 * AbstractScene 
 * 作为Scene的原型的构造函数，保存场景的所有元素（容器，节点和连线），并提供一些基本方法
 * Scene
 * @param {Object} Topo
 * @param {Object} undefined
 */
(function(Topo,undefined){
	...
	function AbstractScene(stage){
        ...
        this.addTodos = function(todos){
            self.todos.push(todos);
        };
        this.clearTodos = function(){
            self.todos = [];
        }
        this.evalTodos = function(){
            var todos = self.todos,
                len, i,todo,graphic = this.graphic;
		...
		this.paint = function(graphic){
			...
			graphic.save();
			if(self.shadow){
				graphic.shadowColor = self.shadow.shadowColor;
				graphic.shadowOffsetX = self.shadow.shadowOffsetX;
				graphic.shadowOffsetY = self.shadow.shadowOffsetY;
				graphic.shadowBlur = self.shadow.shadowBlur;
			}
		...
		this.keydownHelper = function(e){
			this.isKeydown = true;
            this.isCtrlKeyDown = (e.ctrlKey === undefined || e.ctrlKey === false) ? false : true;
			var event = utils.clone(e);
		...
		this.isMouseOverLink = function(link,location){
			...
			lx = location.x;
            ly = location.y;
            var rate = ly*(ex-sx)/(lx*(ey-sy) + ex*sy - sx*ey);
			if(rate >= 0.96 && rate <= 1.04){
			    return true;
			}
			return false;
		}
	...
	function Scene(stage){
		...
		this.initialize = function(stage){
			...
			this.currentElement = null; //记录最近一次选定的元素
			this.mouseoverElement = null;
			this.nodes = [];
			this.links = [];
			this.containers = [];
			this.selectElements = []; //可以存在选定的多个元素
		}
		...
		this.addEventListener('_mousedrag',function(e){
			var event = utils.clone(e),
                selectElements = self.selectElements,
			...
			if(currentElement && currentElement.dragable){
				var ex,ey,nx,ny;
				ex = currentElement.x;
				ey = currentElement.y;
				event.target = currentElement;
				currentElement.onMousedrag(event);
				// 新位置 = 原来位置 + 偏移位置
				nx = currentElement.selectedLocation.x+e.deltaX;
				ny = currentElement.selectedLocation.y+e.deltaY;
				currentElement.setLocation(nx,ny);
				if(selectElements){
					var i,len,el,x,y;
                ...
                self.clearTodos();
                self.addTodos(function(graphic){
                    graphic.save();
                    //        graphic.clearRect(0,0,self.width,self.height);
                    graphic.beginPath();
		...
		this.addEventListener('_mousemove',function(e){
            ...
            var mouseoverElement = self.mouseoverElement;
			self.publish('mousemove',e);
			selectedEl = self.getMouseOverElement(location); 
			if(selectedEl){
				if(mouseoverElement && mouseoverElement !== selectedEl){
					mouseoverElement.onMouseout(event)
				}
                mouseoverElement = self.mouseoverElement = selectedEl;
				selectedEl.onMouseover(event);
				selectedEl.onSelected();
			}else{
				if(mouseoverElement && !utils.isInArray(mouseoverElement,self.selectElements)){
		...
		this.addEventListener('_mousedown',function(e){
            ...
            var selectedElements = self.selectElements,len;
            ...
            //  主要用于多个元素的拖拽
                ...
                event.target = selectedEl;
                // 用于更新所有选中元素的位置信息。
                for(len=selectedElements.length-1;len>=0;len--){
                    selectedElements[len].onSelected();
                }
                selectedEl.onMousedown(event);
			}
			//当被选中的集合中元素数量只有一个或者没有时，则只对当前元素可进行推拽.
                ...
                for(len=selectedElements.length-1;len>=0;len--){
                    selectedElements[len].cancelSelected();
                }
				self.selectElements = [selectedEl];
				self.currentElement = selectedEl;
				event.target = selectedEl;
				selectedEl.onSelected();
				selectedEl.onMousedown(event);
...
(function(Topo,undefined){
	...
	function Link(startNode,endNode,name){
		...
		this.paintSelectedElement = function(graphic){
			...
			graphic.lineWidth = graphic.lineWidth = this.style.lineWidth * 3;
		};
	}
})(Topo);
